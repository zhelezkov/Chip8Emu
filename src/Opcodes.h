...
BEGIN_OPS \
    ...
    OP(op(A,n,n,n), "LD", IAddr, args(number), "Set I = nnn.", fn(0xAnnn)), \
    OP(op(B,n,n,n), "JP", V0Addr, args(number), "Jump to location nnn + V0.", fn(0xBnnn)), \
    ...
    OP(op(F,x,0,7), "LD", VxDT, args(reg), "Set Vx = delay timer value.", fn(0xFx07)), \
